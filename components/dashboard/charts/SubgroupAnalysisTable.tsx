...
export default function SubgroupAnalysisTable({ rawData }: SubgroupAnalysisTableProps) {
  ...
  rawData.forEach((row) => {
    ...
    const confidRaw = Array.isArray(row[26]) ? row[26][0] : row[26]; 
    const afterHoursRaw = Array.isArray(row[19]) ? row[19][0] : row[19]; 
  ...
  const tableData = GROUPS.map((group) => {
    ...
    return {
      group,
      literacy: g.count > 0
        ?`${((g.totalScore / (g.count * 20)) * 100).toFixed(1)}%`: '0%', access: g.count > 0
        ?`${((g.access / g.count) * 100).toFixed(1)}%`: '0%',awareness: g.count > 0
        ?`${((g.awareness / (g.count * 3)) * 100).toFixed(1)}%`: '0%', count: g.count,
    };
  });
